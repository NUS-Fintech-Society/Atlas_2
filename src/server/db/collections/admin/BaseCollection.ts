...
export abstract class BaseCollection<T extends FirebaseFirestore.DocumentData> {
  ...
  withTransaction(transaction: FirebaseFirestore.Transaction) {
    return {
      ...
      update: (id: string, payload: Partial<T>) => transaction.update(this.getDocRef(id) as any, payload as any),
      set: (id: string, payload: T) => transaction.set(this.getDocRef(id), payload),
      delete: (id: string) => transaction.delete(this.getDocRef(id)),
      queryBuilder: () => new FirebaseQueryBuilder<T>(this.collectionName, transaction) 
    }
  }
}
...
class FirebaseQueryBuilder<T> {
  ...
  constructor(collectionName: string, private transaction: FirebaseFirestore.Transaction) {
    this.collectionRef = db.collection(collectionName)
    this.query = this.collectionRef;
  }

  where<K extends keyof T>(field: K, operator: FirebaseFirestore.WhereFilterOp, value: T[K]): FirebaseQueryBuilder<T> {
    this.query = this.query.where(field as string | FirebaseFirestore.FieldPath, operator, value);
    return this;
  }

  orderBy(field: string, direction: FirebaseFirestore.OrderByDirection = 'asc'): FirebaseQueryBuilder<T> {
  ...
  async get(): Promise<FirebaseFirestore.QuerySnapshot<FirebaseFirestore.DocumentData>> {
